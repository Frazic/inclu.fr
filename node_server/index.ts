...
app.post("/join", express.json(), async (req, res) => {
  ...
  if (userInfo.donated) {
    ...
    try {
      ...
      logger.error(
        `${stringifyUserInfo(userInfo)}: Session creation failed: ${error}`
      );
  ...
  if (code || errorMessage) {
    logger.warn(
      `${stringifyUserInfo(
        userInfo
      )}: Insert user failed: { Code: ${code}, Error: ${errorMessage} }`
    );
    res.status(code).json({ error: errorMessage });
    return;
  }
  ...
  async (request, response) => {
    ...
    switch (event.type) {
      case "checkout.session.completed":
        ...
        const { errorMessage } = await insertNewUser({
          name: metadata.name,
        ...
        if (errorMessage) {
          logger.error(
            `CRITICAL: ${metadata} completed checkout but was not added to Supabase: ${errorMessage}`
          );
        }
        break;
...
async function checkUniqueEmail(
  ...
  if (error) {
    logger.error(
      `${stringifyUserInfo(userInfo)}: Select to Supabase failed: ${error}`
    );
...
async function insertNewUser(
  ...
  logger.info(
    `${stringifyUserInfo(userInfo)}: Inserting into Supabase waitlist`
  );
  ...
  if (error)
    logger.error(
      `${stringifyUserInfo(userInfo)}: CRITICAL Couldn't insert into supabase`
    );
  ...
  transporter.sendMail(mailOptions, function (error, info) {
    if (error) {
      logger.error(
        `${stringifyUserInfo(userInfo)}: Error sending email: ${error}`
      );
    } else {
      logger.info(
        `${stringifyUserInfo(userInfo)}: Email sent: ${info.response}`
      );
    }
  });
