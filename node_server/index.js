...
app.listen(process.env.PORT || 8080, function () {
  logger.info(
    "THE SERVER IS WORKING, HERE IS THE PORT: ".concat(process.env.PORT)
  );
});
// CORS
...
app.post("/join", express.json(), function (req, res) {
      ...
      switch (_c.label) {
        ...
        case 1:
          ...
          if (code || errorMessage) {
            logger.warn(
              ""
                .concat(stringifyUserInfo(userInfo), ": { Code: ")
                .concat(code, ", Error: ")
                .concat(errorMessage, " }")
            );
          ...
          logger.info(
            "".concat(stringifyUserInfo(userInfo), ": Donated, going to Stripe")
          );
          _c.label = 2;
        case 2:
          _c.trys.push([2, 4, , 5]);
        ...
        case 3:
          ...
          logger.info(
            "".concat(
              ...
              ": Session creation OK, sending checkout URL"
            )
          );
            ...
            if (errorMessage) {
              logger.error(
                "CRITICAL: "
                  .concat(
                    metadata,
                    " completed checkout but was not added to Supabase: "
                  )
                  .concat(errorMessage)
              );
            }
  ...
  return __awaiter(this, void 0, void 0, function () {
    ...
    return __generator(this, function (_b) {
      switch (_b.label) {
        case 0:
          logger.info(
            "Checking if email is unique: ".concat(stringifyUserInfo(userInfo))
          );
          return [
            4 /*yield*/,
            supabase
        ...
        case 1:
          ...
          if (!(data.length === 0)) {
            ...
            return [
              2 /*return*/,
              { code: 409, errorMessage: "Email déjà enregistré :c" },
            ];
          }
          return [2 /*return*/, {}];
      }
      ...
      switch (_a.label) {
        case 0:
          logger.info(
            "".concat(
              stringifyUserInfo(userInfo),
              ": Inserting into Supabase waitlist"
            )
          );
          return [
            4 /*yield*/,
        ...
        case 1:
          ...
          if (error)
            logger.error(
              "".concat(
                stringifyUserInfo(userInfo),
                ": CRITICAL Couldn't insert into supabase"
              )
            );
          ...
          transporter.sendMail(mailOptions, function (error, info) {
            if (error) {
              ...
              logger.info(
                ""
                  .concat(stringifyUserInfo(userInfo), ": Email sent: ")
                  .concat(info.response)
              );
            }
          });
          if (error) {
